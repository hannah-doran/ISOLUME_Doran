...
class RAMSES:
    ...
    def config_parse(self):
        ...
        for dev in [param[f'dev_{x}'] for x in range(param['n_dev'])]:
            if dev['IDDeviceType'] == 'SAM':
                ...
                self.wl = c0s + np.arange(2, 257) * c1s + np.power(np.arange(2, 257), 2) * c2s + np.power(
                    np.arange(2, 257), 3) * c3s
                self.t0 = 8192
                self.DarkPixelStart = int(dev['DarkPixelStart'])
                self.DarkPixelStop = int(dev['DarkPixelStop'])
                self.bkg1, self.bkg2 = ramses_parse(f'./Ramses Calibration_2017/{self.sn}/Back_{self.id_dev}.dat',
                                                    self.id_dev, datatype='BACK')
                self.air = \
                ramses_parse(f'./Ramses Calibration_2017/{self.sn}/Cal_{self.id_dev}.dat', self.id_dev, datatype='CAL')[
                    1]
                self.aqua = ramses_parse(f'./Ramses Calibration_2017/{self.sn}/CalAQ_{self.id_dev}.dat', self.id_dev,
                                         datatype='CAL')[1]
    ...
    def rx_parse(self, bs, corr=False, plot=False, html=False):
        ...
        for bs in bs_seg:
            ...
            if bs[3] == 0x30 or bs[3] == 0x00:  # SAM
                ...
                self.inclination = 180 / np.pi * np.arctan(
                    np.sqrt(np.tan(x * np.pi / 180) ** 2 + np.tan(y * np.pi / 180) ** 2))  # [degree]
        ...
        if len(raw) == 256:
            ...
            dst_ = np.array(raw, dtype=float).reshape((-1, 1)) / 65535 - (
                        self.bkg1.reshape((-1, 1)) + self.t / self.t0 * self.bkg2)
            if corr:
    ...
    def html_preview(self):
        self.msg = json.dumps({"dev": self.sn,
                               ...
                               "t_exp": self.t,
                               "data": {'x': self.wl.tolist(), 'y': self.cali[1:].flatten().tolist(), 'name': self.sn,
                                        'type': 'scatter'}}).replace('NaN', 'null')
    ...
    def fig_plot(self):
        ...
        if hasattr(self, 'inclination'):
            plt.text(0.6, 0.6,
                     f'Incl. = {self.inclination:.2f}' + r'$^{\circ}$' + f'\nPressure: {self.p_bar:.2f} bar\nWater depth: {self.d_water:.2f} m',
    ...
    def dat_formatter(self, dat_path='./latest.dat'):
        dev_info = {'Version': '1', 'IDData': '', 'IDDevice': self.id_dev, 'IDDataType': 'SPECTRUM',
                    'IDDataTypeSub1': '', 'IDDataTypeSub2': '', 'IDDataTypeSub3': '',
                    'DateTime': time.strftime('%Y-%m-%d %H:%M:%S', time.strptime(self.t_trigger, '%Y%m%d_%H%M%S')),
                    'PositionLatitude': '0', 'PositionLongitude': '0', 'Comment': '', 'CommentSub1': '',
                    'CommentSub2': '', 'CommentSub3': '', 'IDMethodType': f'{self.dev_type} Control',
                    'MethodName': f'{self.dev_type}_{self.sn}', 'Mission': 'No Mission', 'MissionSub': '1',
                    'RecordType': '0'}

        dev_attr = {'CalFactor': '1', 'IDBasisSpec': '', 'IDDataBack': '', 'IDDataCal': '', 'IntegrationTime': self.t,
                    'P31': '-1', 'P31e': '0', 'PathLength': '+INF', 'PathLengthCustomOn': '0', 'RAWDynamic': '65535',
                    'Salinity': '0', 'Temperature': '-NAN', 'Unit1': '', 'Unit2': '', 'Unit3': '',
                    'Unit4': '$f1 $00 Status', 'p999': '1'}
...
if __name__ == "__main__":
    ...
    logging.basicConfig(filename=log_path, filemode='w', format='%(asctime)s %(message)s', datefmt='%Y%m%d_%H%M%S',
                        level=logging.INFO)
    ...
    if len(ramses_dev_list):
        with serial.Serial(port=ramses_dev_list[0][0], baudrate=9600, bytesize=8, parity='N', timeout=1,
                           xonxoff=True) as ser:
